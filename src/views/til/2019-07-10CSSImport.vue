<template>
  <article>
    <h1>Conditional import</h1>
    <blockquote class="twitter-tweet" data-lang="no">
      <p lang="en" dir="ltr">
        I may be the last person to know but
        <a
          href="https://twitter.com/hashtag/TIL?src=hash&amp;ref_src=twsrc%5Etfw"
          rel="noopener"
        >#TIL</a> you can import files based on conditions.
        <a
          href="https://twitter.com/hashtag/CSS?src=hash&amp;ref_src=twsrc%5Etfw"
          rel="noopener"
        >#CSS</a>
        <a href="https://t.co/w0Bd9ZVrHw" rel="noopener">pic.twitter.com/w0Bd9ZVrHw</a>
      </p>&mdash; Ananya Neogi (@_ananyaneogi)
      <a
        href="https://twitter.com/_ananyaneogi/status/1148882328812281856?ref_src=twsrc%5Etfw"
        rel="noopener"
      >10. juli 2019</a>
    </blockquote>
    <script
      type="application/javascript"
      async
      src="https://platform.twitter.com/widgets.js"
      charset="utf-8"
    ></script>
    <p>This is really useful because you can start your mobile css file without the extra indentation.</p>
    <p>Remember that importing other css files like this will make the browser do a seperate network request</p>
  </article>
</template>
